...
class FashionMnistModel(nn.Module):
    ...
    def predict(self, x):
        ...
        return F.softmax(logits) # probablities
...
def main():
    # Create the parser
    parser = argparse.ArgumentParser(description="Fashion Mnist Training and Evaluation.")
    parser.add_argument(
        "--mode", 
        type=str, 
    ...
    if args.mode == "train":
        ...
        for num_epoch in range(args.num_epochs):
            model.train()
            epoch_metrics = {'loss': []}
            for batch in tqdm(train_loader, desc=f"Epoch {num_epoch} | Batch Processing"):
        ...
        print(f"Test Loss: {test_metrics['loss']}", f"Test Accuracy: {test_metrics['accuracy']}")
